...
const OTPStep_Verify = ({ otpCode, setOtpCode, otpError, setOtpError, loading, onVerify, onBack }: OTPVerifyProps) => (
  <SafeAreaView className="flex-1 bg-[#FFF0F3]">
    <KeyboardAwareScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <View className="items-center mb-8">
        <View className="w-20 h-20 bg-white rounded-full items-center justify-center shadow-sm mb-4 border border-rose-100">
      ...
      <View className="mb-6">
        <TextInputField label="Google Authenticator Code" placeholder="000000" keyboardType="number-pad" maxLength={6}
          value={otpCode} onChangeText={(t: string) => { setOtpCode(t.replace(/[^0-9]/g, '')); setOtpError(''); }} error={otpError} />
      </View>
      <Button title="Verify & Login" onPress={onVerify} loading={loading} className="shadow-md bg-[#FF1F4B] rounded-2xl"
        style={{ shadowColor: '#FF1F4B', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8 }} />
      <TouchableOpacity onPress={onBack} className="mt-6 items-center py-3">
...
const QRSetupStep = ({ qrCodeUrl, manualSecret, qrExpired, countdown, setupOtpCode, setSetupOtpCode, loading, onVerify, onRegenerate, onCancel }: QRSetupProps) => {
...
export default function LoginScreen() {
    ...
    if (authStep === 'qr_setup' && qrCodeUrl) {
      ...
      timer = setInterval(() => {
        setCountdown((prev: number) => {
          if (prev <= 1) { if (timer) clearInterval(timer); setQrExpired(true); return 0; }
          return prev - 1;
        });
      }, 1000);
    }
  ...
  const handleSetupFirstOTP = async () => {
    ...
    try {
      ...
      const responseData = err.response?.data;
      const msg = responseData?.message || err.message || 'Invalid code. Please try again.';
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const responseData = err.response?.data;
      const msg = responseData?.message || err.message || 'Login failed. Please check your credentials.';
      ...
      {authStep === 'otp_verify' ? (
        ...
        <KeyboardAwareScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View className="bg-white rounded-3xl p-6 shadow-sm mb-6" style={{ shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 12, elevation: 4 }}>
            ...
            {authStep === 'register' && (
              <>
                <TextInputField label="Full Name" placeholder="John Doe" value={fullName} onChangeText={(t) => { setFullName(t); setErrors((p) => ({ ...p, fullName: undefined })); }} error={errors.fullName} />
                <TextInputField label="Phone" placeholder="+91 98765 43210" keyboardType="phone-pad" value={phone} onChangeText={(t) => { setPhone(t); setErrors((p) => ({ ...p, phone: undefined })); }} error={errors.phone} />
                {/* Role Selection */}
                <View className="mb-4">
            ...
            <TextInputField label="Email" placeholder="email@example.com" keyboardType="email-address" autoCapitalize="none" value={email} onChangeText={(t) => { setEmail(t); setErrors((p) => ({ ...p, email: undefined })); }} error={errors.email} />
            <TextInputField label="Password" placeholder="Enter your password" secureTextEntry={!showPassword} value={password} onChangeText={(t) => { setPassword(t); setErrors((p) => ({ ...p, password: undefined })); }} error={errors.password}
