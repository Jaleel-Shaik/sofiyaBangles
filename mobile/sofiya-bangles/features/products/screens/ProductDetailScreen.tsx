...
export default function ProductDetailScreen() {
  ...
  return (      <View className="flex-1 bg-[#FFF0F3]">
      <View 
        className="absolute left-0 right-0 z-10 flex-row justify-between px-6"
        style={{ top: Math.max(insets.top + 8, 20) }}
      >
        <TouchableOpacity 
          className="w-10 h-10 bg-white/80 rounded-full items-center justify-center shadow-sm border border-white"
          onPress={() => router.back()}
        >
          <Ionicons name="arrow-back" size={24} color="#1e293b" />
        </TouchableOpacity>
        <TouchableOpacity 
          className="w-10 h-10 bg-white/90 rounded-full items-center justify-center shadow-sm border border-slate-100"
