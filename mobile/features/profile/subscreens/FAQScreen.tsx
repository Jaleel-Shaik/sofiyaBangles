...
const FAQItem = ({ question, answer }: { question: string, answer: string }) => {
  ...
  return (
    <TouchableOpacity 
      ...
      className="bg-surface p-5 rounded-2xl mb-4 border border-divider shadow-sm"
    >
      <View className="flex-row justify-between items-center">
        <Text className={`flex-1 pr-4 font-bold ${expanded ? 'text-primary' : 'text-text-primary'}`}>
          {question}
        </Text>
        <Ionicons name={expanded ? "chevron-up" : "chevron-down"} size={20} color={expanded ? "#e11d48" : "#94a3b8"} />
      </View>
      {expanded && (
  ...
  return (
    <View className="flex-1 bg-[#FAFAFA]">
      ...
      >
        ...
        <Text className="text-xl font-bold text-text-primary">Help & FAQs</Text>
      </View>
      ...
      <ScrollView className="flex-1 px-5 pt-6" showsVerticalScrollIndicator={false}>
        ...
        <Text className="text-lg font-bold text-text-primary mb-4">Frequently Asked Questions</Text>
