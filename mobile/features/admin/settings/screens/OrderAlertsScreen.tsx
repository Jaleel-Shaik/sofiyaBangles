...
export default function OrderAlertsScreen() {
  ...
  const AlertItem = ({ title, subtitle, icon, iconBg, iconColor, value, onToggle }: any) => (
    <View className="flex-row items-center p-4 border-b border-divider">
      <View className={`w-12 h-12 rounded-2xl ${iconBg} items-center justify-center mr-4 border border-white shadow-sm`}>
        <Ionicons name={icon} size={22} color={iconColor} />
      </View>
      <View className="flex-1 mr-4">
        <Text variant="body-md" weight="bold" className="text-text-primary mb-0.5">{title}</Text>
        <Text variant="body-sm" className="text-text-secondary">{subtitle}</Text>
      </View>
      <Switch 
        value={value} 
      ...
      >
        <View className="flex-row items-center">
          ...
          <View>
            ...
            <Text variant="title-lg" weight="bold" className="text-text-primary">Order Alerts</Text>
          </View>
        </View>
      </View>

      <ScrollView className="flex-1 px-5 pt-6" showsVerticalScrollIndicator={false}>
        <Text variant="body-sm" weight="bold" className="text-text-secondary mb-3 ml-1 tracking-wider uppercase">Push Notifications</Text>
        <View className="bg-surface rounded-2xl border border-divider mb-8 overflow-hidden shadow-sm">
          <AlertItem 
            title="New Orders" 
        ...
        <Text variant="body-sm" weight="bold" className="text-text-secondary mb-3 ml-1 tracking-wider uppercase">App Experience</Text>
        <View className="bg-surface rounded-2xl border border-divider mb-8 overflow-hidden shadow-sm">
          <AlertItem 
            title="Sound Alerts" 
        ...
        <Text variant="body-sm" weight="bold" className="text-text-secondary mb-3 ml-1 tracking-wider uppercase">Email Summaries</Text>
        <View className="bg-surface rounded-2xl border border-divider mb-8 overflow-hidden shadow-sm">
          <AlertItem 
            title="Daily Digest" 
