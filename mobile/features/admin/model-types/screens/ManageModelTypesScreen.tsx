      ...
      >
        <View className="flex-row items-center">
          ...
          <View>
            ...
            <Text className="text-2xl font-extrabold text-primary">Model Types</Text>
          </View>
        </View>
        {!isAdding && (
          <TouchableOpacity onPress={() => setIsAdding(true)} className="bg-primary px-5 py-2.5 rounded-full">
        ...
        <ScrollView
          ...
          {isAdding && (
            <View className="bg-surface p-5 rounded-2xl mb-6 border border-divider">
              ...
              <Text className="text-xs font-bold text-text-secondary mb-2 uppercase">Model Type Name (e.g. Bangles, Rings)</Text>
              <TextInput 
                value={name}
              ...
              {editingId === mt.id ? (
                <View className="flex-1 mr-2">
                  <TextInput
                    ...
                    className="bg-surface border border-divider rounded-2xl p-3 text-text-primary font-bold mb-2"
                  />
                  <View className="flex-row justify-end">
                    ...
                    <TouchableOpacity onPress={handleUpdate} disabled={updating} className="bg-primary px-4 py-2 rounded-full">
                ...
                <>
                  <Text className="text-lg font-bold text-text-primary flex-1">{mt.name}</Text>
                  <View className="flex-row items-center">
                    <TouchableOpacity onPress={() => startEditing(mt)} className="w-8 h-8 rounded-full bg-surface items-center justify-center mr-2 border border-divider">
                      <Ionicons name="pencil" size={16} color="#e11d48" />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => handleDelete(mt.id)} className="w-8 h-8 rounded-full bg-error/10 items-center justify-center">
