...
export default function ProductsAnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] font-extrabold uppercase tracking-widest bg-rose-50 text-rose-700 border border-rose-200 px-2.5 py-0.5 rounded-full">
              ROLE: PLATFORM SUPER ADMIN (PRODUCT CATALOG & SALES METRICS)
            </span>
          </div>
      ...
      {/* Tabs with explicit label */}
      <div className="flex items-center gap-3 border-b border-slate-200 overflow-x-auto pb-2">
        ...
        <div className="flex items-center gap-2">
          {analyticsTabs.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`px-4 py-2 min-h-[40px] text-xs font-bold rounded-xl transition-all whitespace-nowrap ${
                tab === t.id
