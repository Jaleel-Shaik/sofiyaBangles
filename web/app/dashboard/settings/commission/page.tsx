...
import { Settings, Save, ShieldCheck, Shield } from "lucide-react";
...
export default function CommissionSettingsPage() {
    ...
    return (
      <div className="bg-white border border-rose-200 rounded-2xl p-8 text-center max-w-lg mx-auto mt-10 shadow-sm">
        ...
        <p className="text-xs text-slate-500 mb-6">
          {STRINGS.superAdminAuth.commissionAccessRequiredDesc}
        </p>
        <Link
          href="/dashboard"
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (adminPct + superAdminPct !== 100) {
      toast.error(STRINGS.commissionSettings.sumValidationToast);
      return;
    }
    ...
    try {
      ...
      toast.success(STRINGS.commissionSettings.updateSuccessToast);
    } catch (error: any) {
      toast.error(error?.response?.data?.message || STRINGS.commissionSettings.updateFailedToast);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
          <Settings className="w-6 h-6 text-rose-600" /> {STRINGS.commissionSettings.title}
        </h1>
        <p className="text-xs text-slate-500 mt-1">
          {STRINGS.commissionSettings.subtitle}
        </p>
      </div>
      ...
      <form onSubmit={handleSave} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        {loading ? (
          <p className="text-xs text-slate-400">{STRINGS.commissionSettings.loadingConfig}</p>
        ) : (
          <>
              ...
              <div className="p-5 rounded-2xl bg-emerald-50 border border-emerald-200 space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-emerald-900 uppercase">
                    {STRINGS.commissionSettings.adminSharePercentage}
                  </label>
                ...
                <p className="text-[11px] text-emerald-700">
                  {STRINGS.commissionSettings.adminShareDesc}
                </p>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-rose-50 border border-rose-200 space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-rose-900 uppercase">
                    {STRINGS.commissionSettings.superAdminSharePercentage}
                  </label>
                ...
                <p className="text-[11px] text-rose-700">
                  {STRINGS.commissionSettings.superAdminShareDesc}
                </p>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs font-bold text-slate-700">
              <span className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-emerald-600" /> {STRINGS.commissionSettings.splitCheckTitle}
              </span>
              <span className="text-emerald-700">
                {STRINGS.commissionSettings.splitCheckValid(adminPct, superAdminPct)}
              </span>
            </div>
            ...
            <button
              type="submit"
              disabled={saving}
              className="inline-flex items-center gap-2 bg-rose-600 hover:bg-rose-500 text-white px-6 py-3 min-h-[44px] rounded-xl text-xs font-bold transition-all shadow-md disabled:opacity-50"
            >
              <Save className="w-4 h-4" /> {saving ? STRINGS.commissionSettings.savingButton : STRINGS.commissionSettings.saveButton}
            </button>
          </>
        )}
      </form>
